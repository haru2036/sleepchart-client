...
class SleepChartAdapter(val activity: Activity) : RecyclerView.Adapter<SleepChartAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        view.clearAllSleeps()
        view.onSleepClickListener = {sleep: Sleep -> activity.startActivityForResult(SleepDetailActivity.createIntent(activity, sleep), Constants.SLEEP_DETAIL)}
        view.sleeps = sleepsOfDays.toList()[position].second
        Calendar.getInstance().apply {
            time = view.sleeps.first().end

            val backgroundColor = activity.getColor(
